package dev.nikko.appcanvasfaker.ui.util
...
import dev.nikko.appcanvasfaker.ui.util.AppIconCache.getMainUserId
...
object AppIconCache {
    ...
    /**
     * 缓存键带 config 维度：UI 路径缓存 HARDWARE 位图（hw:），同步编码路径
     * （loadIconSync，供 compress/getPixels 等软件读取）缓存 SOFTWARE 位图（sw:）。
     * 混用同一键会让软件路径拿到 HARDWARE 位图而抛异常，或 UI 路径拿到未优化的软件位图。
     */
    private fun buildKey(applicationInfo: ApplicationInfo, size: Int, hardware: Boolean): String {
        val prefix = if (hardware) "hw" else "sw"
        return "$prefix:${applicationInfo.packageName}:${applicationInfo.uid}:${applicationInfo.sourceDir}:${size}"
    }

    /** UI 展示查询：HARDWARE 优先，SOFTWARE 也可直接绘制。 */
    fun getCached(applicationInfo: ApplicationInfo, size: Int): Bitmap? {
        lruCache.get(buildKey(applicationInfo, size, hardware = true))?.let {
            return it.takeIf { b -> !b.isRecycled }
        }
        return lruCache.get(buildKey(applicationInfo, size, hardware = false))?.takeIf { !it.isRecycled }
    }
    ...
    fun loadIconSync(context: Context, applicationInfo: ApplicationInfo, size: Int): Bitmap {
        val key = buildKey(applicationInfo, size, hardware = false)
    ...
    suspend fun loadIcon(context: Context, applicationInfo: ApplicationInfo, size: Int): Bitmap {
        val key = buildKey(applicationInfo, size, hardware = true)
