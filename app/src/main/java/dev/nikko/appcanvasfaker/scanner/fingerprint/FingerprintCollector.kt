package dev.nikko.appcanvasfaker.scanner.fingerprint
...
object FingerprintCollector {
    ...
    suspend fun collectSync(context: Context): List<FingerprintResult> = withContext(Dispatchers.Default) {
        val standard = dev.nikko.appcanvasfaker.scanner.core.StandardCanvas.createBitmap()
        ...
        run("F1") {
            val baseline = dev.nikko.appcanvasfaker.scanner.core.StandardCanvas.createBitmap()
            try {
