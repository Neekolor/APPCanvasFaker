 ...
 *
 * 背景：远端 prefs 在被 Hook 进程是只读实现，Hook 侧直接写会全灭、计数恒 0，
 * Hook 进程也无其他可写共享通道，故统计回写改走本接收器：
 * Hook 侧发显式广播，本进程收后写本地 prefs.
 *
 * 到达性说明：显式广播可唤醒本进程；本应用被强停期间的命中会丢失
 * （与既有的"允许轻度丢失"约定一致）。伪造说明：Hook 代码即目标应用本身，
...
class StatsReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        runCatching {
            ConfigRepository(context.applicationContext).recordHookHit(pkg, hash, seed, path, ts)
        }
    }
