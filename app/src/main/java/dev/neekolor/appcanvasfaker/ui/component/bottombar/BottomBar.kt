...
import dev.neekolor.appcanvasfaker.ui.component.PagerNavigationSpringSpec
import dev.neekolor.appcanvasfaker.ui.util.shouldShowSplitPane
...
internal fun badgeFor(index: Int, state: NavigationBadgeState): NavBadge? = when (index) {
    BottomBarDestination.AppList.ordinal ->
        state.appListCount.takeIf { it > 0 }?.let { NavBadge(it, BadgeTone.Accent) }
